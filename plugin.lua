...
`با استفاده از این کلید میتوانید نام کانال، یوزرنیم کانال، آی دی کانال، شماره ارتباطی با سامانه پیامکی، انقضای اکانت، آمار کانال، سازنده ی کانال، تعداد ادمینهای کانال، لیست ادمینهای کانال را در یک لیست ببینید.`]]
	...
	elseif msg.text == "مثال کیبرد اینلاین و مارک داون" then
		inlinekey = {{{text = "ربات تیم" , url = "https://telegram.me/suport_araBot"}}}
		text = "مثال استایل نویس با فونتهای مختلف:\n*Ara Bot*\nAra Bot\n_Ara Bot_\n`Ara Bot`\nآرا بات\n`آرا بات`\nمثال هایپر لینک:\n[آرا بات](telegram.me/suport_arabot)\n[Ara Bot](telegram.me/suport_arabot)\nمثال کیبرد اینلاین:"
	...
	if msg.text:find('/add') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/add 12345678,@channelarabot,9121234567*\n`/add [telegram id],[@channel],[number]`", true)
		end
	elseif msg.text:find('/edit') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/edit 12345678,@Channelarabot,9121234567*\n`/edit [telegram id],[@channel],[number]`", true)
		end
	...
	elseif msg.text == "مثال کیبرد اینلاین" then
		return send_inline(msg.from.id, "`تیتر کیبرد اینلاین با قابلیت استفاده از قابلیت مارک داون`", {{{text = "ربات پیام رسان ما" , url = "https://telegram.me/suport_arabot"}}})
	end
